...
import { getFollowingIds, getProfilesByIds, BasicProfile } from '@/services/social';
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const ordered = followingIds.map((uid) => map[uid]).filter(Boolean);
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-950">
      <View className="px-4 pt-3 pb-2 flex-row items-center gap-3">
        ...
        <Text className="text-slate-50 text-xl font-bold">Following</Text>
      </View>
      ...
      {loading ? (
        ...
        <View className="flex-1 items-center justify-center px-6">
          <Text className="text-slate-400 text-center">Not following anyone yet</Text>
        </View>
      ) : (
        <FlatList
          data={people}
